...
// Control table address for Protocol 1.0 (AX & MX series)
...
#define PROTOCOL_VERSION                                                       \
  1.0 // Default Protocol version of DYNAMIXEL Protocol 1.0 series.
...
ReadWriteNode::ReadWriteNode() : Node("read_write_node") {
  ...
  set_position_subscriber_ = this->create_subscription<SetPosition>(
      ...
      [this](const SetPosition::SharedPtr msg) -> void {
        ...
        // Write Goal Position (length : 2 bytes for Protocol 1.0)
  ...
  auto get_present_position =
    ...
    // Read Present Position (length : 2 bytes for Protocol 1.0)
    uint16_t present_pos_16 = 0;
    dxl_comm_result = packetHandler->read2ByteTxRx(
        portHandler, (uint8_t)request->id, ADDR_PRESENT_POSITION,
        &present_pos_16, &dxl_error);

    present_position = (int32_t)present_pos_16;
